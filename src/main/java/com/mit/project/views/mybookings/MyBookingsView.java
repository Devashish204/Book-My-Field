package com.mit.project.views.mybookings;
...
public class MyBookingsView extends Div {
  ...
  private void createClientColumn() {
    clientColumn =
        grid.addColumn(
            ...
            .setComparator(client -> client.getClient())
            .setHeader("Client");
  }
  ...
  private void createAmountColumn() {
    amountColumn =
        grid.addEditColumn(
                Client::getAmount,
                new NumberRenderer<>(
                    client -> client.getAmount(), NumberFormat.getCurrencyInstance(Locale.US)))
        ...
        grid.addEditColumn(
                ...
                new ComponentRenderer<>(
                    client -> {
                      ...
                      span.getElement()
                          .setAttribute("theme", "badge " + client.getStatus().toLowerCase());
                      return span;
                    }))
            .select(
                (item, newValue) -> item.setStatus(newValue),
  ...
  private void createDateColumn() {
    dateColumn =
        grid.addColumn(
            ...
            .setComparator(client -> client.getDate())
            .setHeader("Date")
            .setWidth("180px")
            .setFlexGrow(0);
  }
  ...
  private void addFiltersToGrid() {
    ...
    clientFilter.addValueChangeListener(
        event ->
            gridListDataView.addFilter(
                client ->
                    StringUtils.containsIgnoreCase(client.getClient(), clientFilter.getValue())));
            ...
            gridListDataView.addFilter(
                client ->
                    StringUtils.containsIgnoreCase(
                        Double.toString(client.getAmount()), amountFilter.getValue())));
  ...
  private Client createClient(
      int id, String img, String client, double amount, String status, String date) {
    Client c = new Client();
    c.setId(id);
    c.setImg(img);
    c.setClient(client);
